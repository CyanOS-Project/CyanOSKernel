...
void GDT::setup()
{
	...
	fill_gdt((u32)&gdt_entries, GDT_NUMBER_OF_ENTRIES * sizeof(GDT_ENTRY) - 1);
	...
	// TSS
	fill_gdt_entry(SEGMENT_INDEX(TSS_SELECTOR), u32(&tss_entry), sizeof(TSS_ENTRY), GDT_TSS_PL3, 0);
	// FS and GS
...
void GDT::set_tss_stack(u32 kernel_stack)
{
	memset((void*)&tss_entry, 0, sizeof(TSS_ENTRY));
...
void GDT::setup_tss(u32 kernel_stack)
{
	set_tss_stack(kernel_stack);
	load_tss(TSS_SELECTOR);
}

void GDT::fill_gdt_entry(u32 gdt_entry, u32 base, u32 limit, uint8_t access, uint8_t flags)
{
...
void GDT::fill_gdt(u32 base, u16 limit)
{
	gdt.base = base;
	gdt.limit = limit;
}
